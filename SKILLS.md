...
| `flow_card` | `flow_card(city)` | Flow rate + BAFU level, colour-coded by danger |
...
| `aare_map` | `aare_map(city?)` | Leaflet.js interactive map, all stations, colour-coded by BAFU level |
...
`"olten"`. Tools accept any capitalisation; the client normalises. Use
