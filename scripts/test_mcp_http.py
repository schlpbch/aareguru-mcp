        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if len(temps) == len(cities):
                    ...
                    return True
                print(f"   ❌ Could not get data for all cities")
                return False
        except Exception as e:
            print(f"   ❌ Error: {e}")
            return False
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    if data:  # Just check it's not empty
                        print(f"   ✅ Widget resource available")
                        return True
                print(f"   ❌ Failed: {response.status_code}")
                return False
        except Exception as e:
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    data = response.json()
                    if "aare" in data:
                        print(f"   ✅ Current resource for Bern available")
                        return True
                print(f"   ❌ Failed: {response.status_code}")
                return False
        except Exception as e:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    if "aare" in data or "text" in data:
                        print(f"   ✅ Today resource for Bern available")
                        return True
                print(f"   ❌ Failed: {response.status_code}")
                return False
        except Exception as e:
