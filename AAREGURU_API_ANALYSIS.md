...
The **Aareguru API** is a public, non-commercial API that provides real-time and historical data about the Aare river in Switzerland, including water temperature, flow rates, weather conditions, and forecasts. The API is maintained by Christian Studer and Aare.guru GmbH, serving the popular [Aare.guru](https://aare.guru/) service.
...
This simple stack ensures high availability and easy maintenance for a hobby project.
...
**Use Case**: Discovery endpoint to find available locations and their identifiers
...
**Use Case**: Lightweight endpoint for simple integrations (e.g., IoT devices, status displays)
...
**Response**: Medium-sized response with current data and forecasts for all cities

**Use Case**: Widgets, dashboards, or applications displaying multiple locations simultaneously
...
- `start` (required): Start date/time in various formats (ISO, timestamp, relative like "-1 day")
...
**Automatic Period Selection**: The API automatically chooses the appropriate data granularity based on the requested time range
...
The `values` parameter allows extracting specific fields as plain text (one per line):
...
> [!IMPORTANT]
> This API is free for non-commercial use with the following requirements:
...
- **Newsletter**: [API News Mailing List](mailto:cstuder@existenz.ch?subject=Newsletter%20Aare.guru%20API)
- **RSS Feed**: [Newsletter Tag](https://hymnos.existenz.ch/tag/newsletter/feed/)

---

## Security

- **Security Policy**: Available at `https://aareguru.existenz.ch/.well-known/security.txt`
...
The version number represents the year of the major API revision. This approach provides:
...
Based on the [Aare Guru Techstack Webinar](https://hymnos.existenz.ch/2021/12/03/video-aare-guru-techstack-webinar/):
...
- **Developer**: Christian Studer ([cstuder@existenz.ch](mailto:cstuder@existenz.ch))
- **Company**: [Aare.guru GmbH](https://firma.aare.guru)
- **Related**: [Opendata.ch](https://opendata.ch) - Swiss Open Knowledge Foundation

---

## Conclusion

The Aareguru API is a well-designed, reliable public API that successfully balances simplicity with functionality. Its strengths include:
...
5. **Sustainable Architecture**: Simple technology stack for long-term maintenance
...
*API Version Analyzed: v2018*  
*Documentation Sources: OpenAPI Specification + Official Parameter Documentation*
