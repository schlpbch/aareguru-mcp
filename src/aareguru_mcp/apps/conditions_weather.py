...
def render_weather_section(
    weather: dict[str, Any] | None = None, lang: str = "de"
) -> None:
    ...
    with Card(
        css_class=f"bg-[{_AG_BG_WETTER}] dark:bg-[{_DK.BG_WETTER}] {_AG_RADIUS} overflow-hidden"
    ):
        with CardContent(css_class="p-3"):
            ...
            with Row(css_class="items-center gap-2 mb-2"):
                _sy_to_icon(sy, css_class="w-8 h-8")
                if syt:
                    Text(
                        syt,
                        css_class=f"text-sm font-semibold text-[{_AG_TXT_PRIMARY}] dark:text-[{_DK.TXT_PRIMARY}]",
                    )

            with Grid(columns=3, gap=0):
                # Air temp + min/max
                with Card(
                    css_class=f"{_AG_RADIUS} bg-[{_AG_BG_WETTER}]/60 dark:bg-[{_DK.BG_WETTER}]/80"
                ):
                    with CardContent(css_class="p-2 text-center"):
                        ...
                        Text(
                            ...
                            css_class=f"text-xl font-black tabular-nums text-[{_AG_AIR_TEMP}] dark:text-[{_DK.AIR_TEMP}]",
                        )
                        Muted(
                        ...
                        if tn is not None or tx is not None:
                            Muted(
                                ...
                                css_class=f"text-[10px] text-[{_AG_AIR_TEMP}]/70 dark:text-[{_DK.AIR_TEMP}]/70",
                            )
                ...
                with Card(
                    ...
                    with CardContent(css_class="p-2 text-center"):
                        Text(
                            ...
                            css_class=f"text-xl font-black tabular-nums text-[{_AG_TXT_PRIMARY}] dark:text-[{_DK.TXT_PRIMARY}]",
                        )
                        Muted(
                ...
                with Row(css_class="gap-0 overflow-x-auto"):
                    for entry in forecast_list[:6]:
                        ...
                        with Card(
                            css_class=f"{_AG_RADIUS} bg-[{_AG_BG_WETTER}]/60 dark:bg-[{_DK.BG_WETTER}]/50 min-w-[52px] flex-shrink-0"
                        ):
                            with CardContent(css_class="p-1.5 text-center"):
                                ...
                                _sy_to_icon(entry_sy, css_class="w-5 h-5 my-0.5")
                                Text(
                                    _fmt_temp(entry_tt),
                                    css_class=f"text-xs font-bold text-[{_AG_AIR_TEMP}] dark:text-[{_DK.AIR_TEMP}] tabular-nums",
                                )
